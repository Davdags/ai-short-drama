...
const prismaMock = vi.hoisted(() => ({
  ...
  studioCharacter: { create: vi.fn(async () => ({ id: 'char-new-1' })) },
  studioLocation: { create: vi.fn(async () => ({ id: 'loc-new-1' })) },
  locationImage: {
    create: vi.fn(async () => ({})),
...
describe('worker analyze-novel behavior', () => {
  beforeEach(() => {
    ...
    prismaMock.studioLocation.create
    ...
    prismaMock.studioEpisode.findFirst.mockResolvedValue({
      novelText: '首集内容',
    })
  ...
  it('no global text and no episode text -> explicit error', async () => {
    prismaMock.studioProject.findUnique.mockResolvedValueOnce({
      id: 'np-project-1',
      analysisModel: 'llm::analysis-1',
      artStyle: 'cinematic',
      globalAssetText: '',
    ...
    prismaMock.studioEpisode.findFirst.mockResolvedValueOnce({ novelText: '' })

    await expect(handleAnalyzeNovelTask(buildJob())).rejects.toThrow('Please add global settings or story text first')
  })
  ...
  it('success path -> creates character/location and persists cleaned location descriptions', async () => {
    ...
    expect(prismaMock.studioCharacter.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          studioProjectId: 'np-project-1',
          name: '新角色',
    ...
    expect(prismaMock.studioLocation.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          studioProjectId: 'np-project-1',
          name: '新地点',
