...
function buildJob(params: {
  ...
  return {
    data: {
      ...
      projectId: 'project-1',
      episodeId: 'episode-1',
      targetType: params.targetType ?? 'StudioPanel',
...
describe('worker video processor behavior', () => {
  beforeEach(async () => {
    ...
    prismaMock.studioPanel.findUnique.mockResolvedValue(buildPanel())
    prismaMock.studioPanel.findFirst.mockResolvedValue(buildPanel())
    prismaMock.studioVoiceLine.findUnique.mockResolvedValue({
      id: 'line-1',
  ...
  it('LIP_SYNC: 缺少 panel 时显式失败', async () => {
    ...
    prismaMock.studioPanel.findUnique.mockResolvedValueOnce(null)
    const job = buildJob({
  ...
  it('LIP_SYNC: 正常路径写回 lipSyncVideoUrl 并清理 lipSyncTaskId', async () => {
    ...
    expect(prismaMock.studioPanel.update).toHaveBeenCalledWith({
      where: { id: 'panel-1' },
      data: {
        lipSyncVideoUrl: 'cos/lip-sync/video.mp4',
        lipSyncTaskId: null,
