...
const prismaMock = vi.hoisted(() => ({
  project: {
    findUnique: vi.fn(async () => ({ id: 'project-1', mode: 'studio' })),
  },
  studioProject: {
...
vi.mock('@/lib/studio/story-to-script/clip-matching', () => ({
...
describe('worker episode-split', () => {
  ...
  it('fails fast when content is too short', async () => {
    ...
    await expect(handleEpisodeSplitTask(job)).rejects.toThrow('The text is too short. At least 100 characters are needed')
  })
