...
import { useWorkspaceVideoActions } from '@/app/[locale]/workspace/[projectId]/modes/studio/hooks/useWorkspaceVideoActions'
...
describe('useWorkspaceVideoActions', () => {
  ...
  it('single video mutation fails -> rethrows error for immediate lock cleanup', async () => {
    ...
    expect(notifyMock.notifyAlert).toHaveBeenCalledWith('execution.generationFailed: video submit failed')
  })
})
