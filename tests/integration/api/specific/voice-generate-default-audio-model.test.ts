...
const authMock = vi.hoisted(() => ({
  requireProjectAuthLight: vi.fn(async () => ({
    ...
    project: { id: 'project-1', userId: 'user-1', mode: 'studio' },
  })),
...
describe('api specific - voice generate default audio model', () => {
  ...
  it('uses project audioModel when request does not provide one', async () => {
    const mod = await import('@/app/api/studio/[projectId]/voice-generate/route')
    const req = buildMockRequest({
      path: '/api/studio/project-1/voice-generate',
      method: 'POST',
      body: {
        episodeId: 'episode-1',
        lineId: 'line-1',
