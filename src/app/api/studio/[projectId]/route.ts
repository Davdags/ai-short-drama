...
export const PATCH = apiHandler(async (
  ...
  const currentProjectConfig = await prisma.studioProject.findUnique({
    where: { projectId },
    select: {
      analysisModel: true,
      characterModel: true,
  ...
  const updatedStudioData = await prisma.studioProject.update({
    where: { projectId },
    data: updateData})

  const studioDataWithSignedUrls = await attachMediaFieldsToProject(updatedStudioData)

  const fullProject = {
    ...project,
    studioData: studioDataWithSignedUrls}
