...
export const POST = apiHandler(async (
  ...
  const episode = await prisma.studioEpisode.findFirst({
    where: {
      id: episodeId,
      studioProjectId: projectData.id},
    select: {
      id: true,
      speakerVoices: true}})
  if (!episode) {
  ...
  if (all) {
    const allLines = await prisma.studioVoiceLine.findMany({
      where: {
        episodeId,
        audioUrl: null},
    ...
    const line = await prisma.studioVoiceLine.findFirst({
      where: {
        id: lineId,
        episodeId},
      select: {
  ...
  if (voiceLines.length === 0) {
    if (all) {
      const firstLineWithoutBinding = await prisma.studioVoiceLine.findFirst({
        where: {
          episodeId,
          audioUrl: null,
        },
