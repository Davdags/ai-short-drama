...
export const DELETE = apiHandler(async (
  ...
  await prisma.studioCharacter.delete({
    where: { id: characterId }
  })
...
export const POST = apiHandler(async (
  ...
  // 创建角色
  const character = await prisma.studioCharacter.create({
    data: {
  ...
  // 创建初始形象（独立表）
  const descText = description || `Character profile for ${name}`
  ...
  if (generateFromReference && allReferenceImages.length > 0) {
    ...
    fetch(`${baseUrl}/api/studio/${projectId}/reference-to-character`, {
      method: 'POST',
      headers: {
  ...
  // 返回包含形象的角色数据
  const characterWithAppearances = await prisma.studioCharacter.findUnique({
