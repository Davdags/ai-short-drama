...
async function validateVideoCapabilityCombination(input: {
  ...
  // Filter selections to only include fields the model's catalog actually defines
  const videoCaps = (builtinCaps as Record<string, Record<string, unknown>>)?.video ?? {}
  const supportedFields = new Set(
    Object.keys(videoCaps).filter(k => k.endsWith('Options')).map(k => k.slice(0, -7)),
  )
  for (const key of Object.keys(runtimeSelections)) {
    if (!supportedFields.has(key)) delete runtimeSelections[key]
  }
...
export const POST = apiHandler(async (
  ...
  if (isBatch) {
    ...
    const panels = await prisma.studioPanel.findMany({
      where: {
  ...
  const panel = await prisma.studioPanel.findFirst({
