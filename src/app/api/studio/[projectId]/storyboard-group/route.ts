...
/**
 * POST /api/studio/[projectId]/storyboard-group
...
export const POST = apiHandler(async (
  ...
  const result = await prisma.$transaction(async (tx) => {
    // 1. 创建新的 Clip（手动添加类型）
    const newClip = await tx.studioClip.create({
      data: {
        episodeId,
    ...
    // 2. 创建关联的 Storyboard
    const newStoryboard = await tx.studioStoryboard.create({
      data: {
        episodeId,
        clipId: newClip.id,
        panelCount: 1
    ...
    // 3. 创建初始的 Panel
    const newPanel = await tx.studioPanel.create({
      data: {
...
/**
 * PUT /api/studio/[projectId]/storyboard-group
...
/**
 * DELETE /api/studio/[projectId]/storyboard-group
...
export const DELETE = apiHandler(async (
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // 2. 删除 Storyboard
    await tx.studioStoryboard.delete({
      where: { id: storyboardId }
    })

    // 3. 删除关联的 Clip（如果存在）
    if (storyboard.clipId) {
      await tx.studioClip.delete({
