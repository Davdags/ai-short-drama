...
// ── EvoLink Default Models ──
// Text: Gemini 3.1 Pro — best balance of quality, speed, and cost for analysis
const DEFAULT_ANALYSIS_MODEL_KEY = composeModelKey('evolink', 'gemini-3.1-pro-preview')
// Image: NanoBanana Pro — highest quality image gen on EvoLink
const DEFAULT_IMAGE_MODEL_KEY = composeModelKey('evolink', 'gemini-3-pro-image-preview')
// Video: Seedance 2.0 — smart routing (auto t2v/i2v/ref2v), supports first/last frame, audio
const DEFAULT_VIDEO_MODEL_KEY = composeModelKey('evolink', 'seedance-2.0')
// Audio TTS
const DEFAULT_AUDIO_MODEL_KEY = composeModelKey('evolink', 'qwen3-tts-vd')
// Voice Design
const DEFAULT_VOICE_DESIGN_MODEL_KEY = composeModelKey('evolink', 'qwen-voice-design')
// Lip Sync
const DEFAULT_LIPSYNC_MODEL_KEY = composeModelKey('evolink', 'videoretalk')
...
function validateModelProviderConsistency(models: StoredModel[], providers: StoredProvider[]) {
  for (let index = 0; index < models.length; index += 1) {
    ...
    // EvoLink models are auto-injected; skip provider check
    if (getProviderKey(model.provider) === 'evolink') continue
...
function validateModelProviderTypeSupport(models: StoredModel[], providers: StoredProvider[]) {
  for (let index = 0; index < models.length; index += 1) {
    ...
    if (model.type === 'lipsync' && providerKey !== 'fal' && providerKey !== 'vidu' && providerKey !== 'bailian' && providerKey !== 'evolink') {
      throw new ApiError('INVALID_PARAMS', {
...
export const GET = apiHandler(async () => {
  ...
  for (const p of central ? [] : providers) {
    ...
    for (const preset of EVOLINK_MODEL_PRESETS) {
      ...
      pricedModels.push(withDisplayPricing(base, pricingDisplay))
    }
  }

  const rawDefaults: DefaultModelsPayload = {
    analysisModel: pref?.analysisModel || DEFAULT_ANALYSIS_MODEL_KEY,
    characterModel: pref?.characterModel || DEFAULT_IMAGE_MODEL_KEY,
    locationModel: pref?.locationModel || DEFAULT_IMAGE_MODEL_KEY,
    storyboardModel: pref?.storyboardModel || DEFAULT_IMAGE_MODEL_KEY,
    editModel: pref?.editModel || DEFAULT_IMAGE_MODEL_KEY,
    videoModel: pref?.videoModel || DEFAULT_VIDEO_MODEL_KEY,
    audioModel: pref?.audioModel || DEFAULT_AUDIO_MODEL_KEY,
    lipSyncModel: pref?.lipSyncModel || DEFAULT_LIPSYNC_MODEL_KEY,
    voiceDesignModel: pref?.voiceDesignModel || DEFAULT_VOICE_DESIGN_MODEL_KEY,
  }
