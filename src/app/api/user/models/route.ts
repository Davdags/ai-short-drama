...
export const GET = apiHandler(async () => {
  ...
  // Track existing model keys to avoid duplicates with presets
  ...
  // Inject EvoLink presets for providers with API keys
  for (const provider of providers) {
    const providerId = typeof provider?.id === 'string' ? provider.id.trim() : ''
    if (!providerId || getProviderKey(providerId) !== 'evolink') continue
    if (!hasStoredProviderApiKey(provider)) continue

    for (const preset of EVOLINK_MODEL_PRESETS) {
      const modelKey = composeModelKey(providerId, preset.modelId)
      if (!modelKey || seenModelKeys.has(modelKey)) continue
      if (!isUnifiedModelType(preset.type)) continue
      ...
      const option: UserModelOption = {
        ...
        label: preset.name,
        provider: providerId,
        providerName: providerNameMap.get(providerId),
      }

      const capabilities = findBuiltinCapabilities(preset.type, providerId, preset.modelId)
      if (capabilities) {
        option.capabilities = capabilities
      }

      if (preset.type === 'video') {
        const pricingEntry = findBuiltinPricingCatalogEntry('video', providerId, preset.modelId)
        if (pricingEntry?.pricing.mode === 'capability' && Array.isArray(pricingEntry.pricing.tiers)) {
          option.videoPricingTiers = cloneVideoPricingTiers(pricingEntry.pricing.tiers)
        }
      }
