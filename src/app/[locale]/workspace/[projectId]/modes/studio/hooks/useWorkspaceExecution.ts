...
export function useWorkspaceExecution({
  ...
  const storyToScriptMinimizedStorageKey = `studio:story-to-script:minimized:${storageScope}`
  const scriptToStoryboardMinimizedStorageKey = `studio:script-to-storyboard:minimized:${storageScope}`
  ...
  const handleGenerateTTS = useCallback(async () => {
    _ulogInfo('[StudioWorkspace] TTS is disabled, skip generate request')
  }, [])
