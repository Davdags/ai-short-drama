...
import { StudioPanel } from '@/types/project'
...
interface StoryboardPanelListProps {
  ...
  getPanelCandidates: (panel: StudioPanel) => { candidates: string[]; selectedIndex: number } | null
...
export default function StoryboardPanelList({
  ...
  return (
    <div className={`grid gap-4 ${isVertical ? 'grid-cols-1 sm:grid-cols-3 lg:grid-cols-5' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'} ${isSubmittingStoryboardTextTask ? 'opacity-50 pointer-events-none' : ''}`}>
      {textPanels.map((panel, index) => {
        ...
        const panelCandidateData = getPanelCandidates(panel as unknown as StudioPanel)
