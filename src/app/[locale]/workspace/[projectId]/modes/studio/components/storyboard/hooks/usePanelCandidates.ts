...
import type { StudioPanel } from '@/types/project'
...
export function usePanelCandidates({
  ...
  const handleEnsurePanelCandidatesInitialized = useCallback((panel: StudioPanel): boolean => {
    return ensurePanelCandidatesInitialized(panel, candidateSystem)
  }, [candidateSystem])

  const getPanelCandidates = useCallback((panel: StudioPanel): PanelCandidateData | null => {
  ...
  const hasPanelCandidates = useCallback((panel: StudioPanel): boolean => {
