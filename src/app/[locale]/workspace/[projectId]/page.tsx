...
import StudioWorkspace from './modes/studio/StudioWorkspace'
import SmartImportWizard, { SplitEpisode } from './modes/studio/components/SmartImportWizard'
...
export default function ProjectDetailPage() {
  ...
  // 获取剧集列表
  const studioData = project?.studioData as StudioData | undefined
  const episodes = useMemo<Episode[]>(() => {
    const getNum = (name: string) => { const m = name.match(/\d+/); return m ? parseInt(m[0], 10) : Infinity }
    return [...(studioData?.episodes ?? [])].sort((a, b) => {
  ...
  // 获取导入状态
  const importStatus = studioData?.importStatus
  ...
  const handleCreateEpisode = async (name: string, description?: string) => {
    const res = await apiFetch(`/api/studio/${projectId}/episodes`, {
      method: 'POST',
  ...
  const handleSmartImportComplete = async (splitEpisodes: SplitEpisode[], triggerGlobalAnalysis?: boolean) => {
    ...
    try {
      ...
      const data = await res.json()
      // API 返回结构是 { project: { studioData: { episodes: [...] } } }
      const newEpisodes = data?.project?.studioData?.episodes || []
  ...
  const handleRenameEpisode = async (episodeId: string, newName: string) => {
    const res = await apiFetch(`/api/studio/${projectId}/episodes/${episodeId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <main className="flex-1 min-h-0 overflow-hidden">
          {isGlobalAssetsView && project.studioData ? (
            // 全局资产视图（确保数据准备好）
            <div>
