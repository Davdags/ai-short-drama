...
function normalizeMoney(value: number): number {
  ...
  return roundMoney(Math.max(0, numeric), PRECISION_DIGITS)
}
...
function clampChargedCost(actualCost: number, freezeCost: number) {
  ...
  if (normalizedActual <= normalizedFreeze + ROUNDING_EPSILON) {
...
async function ensureFreezeCoverage(params: {
  ...
  if (chargedCost <= normalizedQuoted + ROUNDING_EPSILON) {
    return chargedCost
  }

  const overage = normalizeMoney(chargedCost - normalizedQuoted)
  if (overage <= ROUNDING_EPSILON) {
    return chargedCost
  }
