...
function resolveProviderKey(value: string): LipSyncProviderKey {
  ...
  if (providerKey === 'fal' || providerKey === 'vidu' || providerKey === 'bailian' || providerKey === 'evolink') {
    return providerKey
  }
...
export async function generateLipSync(
  ...
  try {
    ...
    if (providerKey === 'evolink') {
      const result = await submitEvolinkLipSync(preprocessedParams, context)
      _ulogInfo(`[LipSync Async] EvoLink lip sync submitted: ${result.requestId}`)
      return result
    }
