...
/**
 * 验证项目访问权限
 * 包含：Session 验证 + 项目存在检查 + 所有权验证 + StudioData 检查
 * 
...
export async function requireProjectAuth<T extends ProjectAuthIncludes = ProjectAuthIncludes>(
    ...
    // 2. 构建动态 include 对象
    const studioIncludes: Record<string, boolean> = {}
    if (options?.include?.characters) {
        studioIncludes.characters = true
    }
    if (options?.include?.locations) {
        studioIncludes.locations = true
    }
    if (options?.include?.episodes) {
        studioIncludes.episodes = true
    }

    // 3. 获取项目（包含 studioData 及其可选关联）
    const hasIncludes = Object.keys(studioIncludes).length > 0
    const project = await withPrismaRetry(() =>
        prisma.project.findUnique({
            where: { id: projectId },
            include: {
                studioData: hasIncludes
                    ? { include: studioIncludes }
                    : true
            }
        })
    )
    ...
    // 6. StudioData 检查
    if (!project.studioData) {
        return notFound('Studio data')
    }

    // 统一返回 modelKey（provider::modelId），禁止降级为纯 modelId
    const rawNovelData = project.studioData as {
...
/**
 * 验证项目权限（不要求 StudioData）
 * 适用于某些不需要 studioData 的 API
 */
