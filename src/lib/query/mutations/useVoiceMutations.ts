    ...
    return useMutation({
        mutationFn: async (payload: {
            ...
            const response = await requestTaskResponseWithError(
                `/api/studio/${projectId}/voice-design`,
                {
                    method: 'POST',
...
export function useAnalyzeProjectVoice(projectId: string) {
    return useMutation({
        mutationFn: async ({ episodeId }: { episodeId: string }) => {
            const response = await requestTaskResponseWithError(
                `/api/studio/${projectId}/voice-analyze`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    return useMutation({
        mutationFn: async ({
            ...
            await requestJsonWithError<GenerateProjectVoiceResponse>(
                `/api/studio/${projectId}/voice-generate`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    return useMutation({
        mutationFn: async (payload: {
            ...
            await requestJsonWithError<{ voiceLine: ProjectVoiceLine }>(
                `/api/studio/${projectId}/voice-lines`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload),
    ...
    return useMutation({
        mutationFn: async ({ lineId }: { lineId: string }) => {
            await requestVoidWithError(
                `/api/studio/${projectId}/voice-lines?lineId=${lineId}`,
                { method: 'DELETE' },
                'delete failed',
            )
            return null
    ...
    return useMutation({
        mutationFn: async ({ episodeId }: { episodeId: string }) =>
            await requestBlobWithError(
                `/api/studio/${projectId}/download-voices?episodeId=${episodeId}`,
                { method: 'GET' },
                'download failed',
            ),
    })
    ...
    return useMutation({
        mutationFn: async (payload: {
            ...
            await requestJsonWithError<{ success: boolean }>(
                `/api/studio/${projectId}/speaker-voice`,
                {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload),
