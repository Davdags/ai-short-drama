...
export function useUploadProjectCharacterVoice(projectId: string) {
    ...
    return useMutation({
        mutationFn: async ({ file, characterId }: { file: File; characterId: string }) => {
            ...
            return await requestJsonWithError(`/api/studio/${projectId}/character-voice`, {
                method: 'POST',
                body: formData
...
export function useSaveProjectDesignedVoice(projectId: string) {
    ...
    return useMutation({
        mutationFn: async ({
            ...
            return await requestJsonWithError<{ audioUrl?: string }>(`/api/studio/${projectId}/character-voice`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    characterId,
