  ...
  return useMutation({
    mutationFn: async (payload: { content: string; async?: boolean }) => {
      const response = await requestTaskResponseWithError(
        `/api/studio/${projectId}/episodes/split`,
        {
          method: 'POST',
  ...
  return useMutation({
    mutationFn: async (payload: { content: string }) =>
      await requestJsonWithError<{
        ...
        `/api/studio/${projectId}/episodes/split-by-markers`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
...
export function useSaveProjectEpisodesBatch(projectId: string) {
  return useMutation({
    mutationFn: async (payload: {
      ...
      await requestJsonWithError(
        `/api/studio/${projectId}/episodes/batch`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
...
export function useUpdateProjectEpisodeField(projectId: string) {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      await requestJsonWithError(
        `/api/studio/${projectId}/episodes/${episodeId}`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
    ...
    onMutate: async (variables) => {
      ...
      queryClient.setQueryData<Project | undefined>(projectQueryKey, (prev) => {
        if (!prev?.studioData) return prev
        const episodes = Array.isArray(prev.studioData.episodes)
          ? prev.studioData.episodes.map((episode) =>
              episode.id === variables.episodeId ? { ...episode, [variables.key]: variables.value } : episode,
            )
          : prev.studioData.episodes
        return {
          ...prev,
          studioData: {
            ...prev.studioData,
...
export function useUpdateProjectClip(projectId: string) {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      await requestJsonWithError(
        `/api/studio/${projectId}/clips/${clipId}`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
