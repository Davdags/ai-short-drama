...
function removeLocationFromProject(
    ...
    if (!previous?.studioData) return previous
    const currentLocations = previous.studioData.locations || []
    return {
        ...previous,
        studioData: {
            ...previous.studioData,
    ...
    return useMutation({
        mutationFn: async (locationId: string) => {
            await requestVoidWithError(
                `/api/studio/${projectId}/location?id=${encodeURIComponent(locationId)}`,
                { method: 'DELETE' },
...
export function useUpdateProjectLocationDescription(projectId: string) {
    ...
    return useMutation({
        mutationFn: async ({
            ...
            return await requestJsonWithError(`/api/studio/${projectId}/location`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    locationId,
    ...
    return useMutation({
        mutationFn: async ({
            ...
            const response = await requestTaskResponseWithError(
                `/api/studio/${projectId}/ai-modify-location`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
...
export function useAiCreateProjectLocation(projectId: string) {
    return useMutation({
        mutationFn: async (payload: { userInstruction: string }) => {
            const response = await requestTaskResponseWithError(
                `/api/studio/${projectId}/ai-create-location`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
...
export function useConfirmProjectLocationSelection(projectId: string) {
    ...
    return useMutation({
        mutationFn: async ({ locationId }: { locationId: string }) =>
            await requestJsonWithError(
                `/api/studio/${projectId}/location/confirm-selection`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
