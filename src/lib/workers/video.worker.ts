...
type PanelRecord = NonNullable<Awaited<ReturnType<typeof prisma.studioPanel.findUnique>>>
...
async function fetchPanelByStoryboardIndex(storyboardId: string, panelIndex: number) {
  return await prisma.studioPanel.findFirst({
    where: {
      storyboardId,
      panelIndex,
    },
...
async function getPanelForVideoTask(job: Job<TaskJobData>) {
  ...
  // 优先使用 targetType=StudioPanel 直接定位
  if (job.data.targetType === 'StudioPanel') {
    const panel = await prisma.studioPanel.findUnique({ where: { id: job.data.targetId } })
...
async function handleVideoPanelTask(job: Job<TaskJobData>) {
  ...
  await prisma.studioPanel.update({
    where: { id: panel.id },
    data: {
      videoUrl: cosKey,
...
async function handleLipSyncTask(job: Job<TaskJobData>) {
  ...
  if (job.data.targetType === 'StudioPanel') {
    panel = await prisma.studioPanel.findUnique({ where: { id: job.data.targetId } })
  }
  ...
  const voiceLine = await prisma.studioVoiceLine.findUnique({ where: { id: voiceLineId } })
