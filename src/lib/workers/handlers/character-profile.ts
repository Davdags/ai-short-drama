...
async function handleConfirmProfile(
  ...
  const character = await prisma.studioCharacter.findFirst({
    where: {
      id: characterId,
      studioProjectId: project.studioData!.id,
    },
  })
  if (!character) {
  ...
  if (payload.profileData) {
    if (!validateProfileData(payload.profileData)) {
      throw new Error('Character profile data is invalid')
    }
  ...
  if (!finalProfileData) {
    throw new Error('This character has no profile data')
  }
  ...
  if (!suppressProgress) {
    await reportTaskProgress(job, 20, {
      ...
      stageLabel: 'Preparing character profile',
      displayMode: 'detail',
    })
  }
  ...
  const completion = await withInternalLLMStreamCallbacks(
    ...
    async () =>
      await executeAiTextStep({
        ...
        model: project.studioData!.analysisModel!,
  ...
  if (appearances.length === 0) {
    throw new Error('The AI returned an invalid format (missing appearances)')
  }

  if (!suppressProgress) {
    await reportTaskProgress(job, 78, {
      stage: 'character_profile_confirm_persist',
      stageLabel: 'Saving character profile',
      displayMode: 'detail',
    })
  }
  ...
  for (let appIndex = 0; appIndex < appearances.length; appIndex++) {
    ...
    appearanceRows.push({
      ...
      changeReason: readText(app.change_reason) || 'Initial look',
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.studioCharacter.update({
      where: { id: characterId },
      data: {
  ...
  if (!suppressProgress) {
    await reportTaskProgress(job, 96, {
      ...
      stageLabel: 'Character profile confirmed',
      displayMode: 'detail',
      meta: { characterId },
    })
  }
...
async function handleBatchConfirmProfile(job: Job<TaskJobData>) {
  ...
  const unconfirmedCharacters = await prisma.studioCharacter.findMany({
    where: {
      studioProjectId: project.studioData!.id,
  ...
  if (unconfirmedCharacters.length === 0) {
    return {
      success: true,
      count: 0,
      message: 'No characters need confirming',
    }
  }

  await reportTaskProgress(job, 18, {
    stage: 'character_profile_batch_prepare',
    stageLabel: 'Preparing character profiles',
    displayMode: 'detail',
    message: `${unconfirmedCharacters.length} character(s)`,
  })
  ...
  for (let index = 0; index < unconfirmedCharacters.length; index++) {
    ...
    await reportTaskProgress(job, progress, {
      ...
      stageLabel: 'Confirming character profiles',
      displayMode: 'detail',
  ...
  await reportTaskProgress(job, 96, {
    ...
    stageLabel: 'Character profiles confirmed',
    displayMode: 'detail',
