...
export async function handleReferenceToCharacterTask(job: Job<TaskJobData>) {
  ...
  await reportTaskProgress(job, 15, {
    ...
    stageLabel: 'Preparing reference conversion',
    displayMode: 'detail',
  })
  ...
  if (!imageModel && !extractOnly) {
    throw new Error('Please choose a character image model in Model preferences first')
  }
  ...
  if (extractOnly) {
    await reportTaskProgress(job, 45, {
      ...
      stageLabel: 'Describing the reference image',
      displayMode: 'detail',
    })
    ...
    await reportTaskProgress(job, 96, {
      ...
      stageLabel: 'Reference described',
      displayMode: 'detail',
    })
    return {
      success: true,
  ...
  await reportTaskProgress(job, 35, {
    ...
    stageLabel: 'Generating character turnaround',
    displayMode: 'detail',
  })
  ...
  if (isBackgroundJob && appearanceId) {
    ...
    await reportTaskProgress(job, 96, {
      ...
      stageLabel: 'Reference conversion complete',
      displayMode: 'detail',
    })
    return { success: true }
  }
