...
export async function handleVoiceAnalyzeTask(job: Job<TaskJobData>) {
  ...
  if (episode.studioProjectId !== studioData.id) {
  ...
  const analysisModel = await resolveAnalysisModel({
    ...
    projectAnalysisModel: studioData.analysisModel,
  })

  const charactersLibName = studioData.characters.length > 0
    ? studioData.characters.map((c) => c.name).join('、')
    : 'None'
  const charactersIntroduction = buildCharactersIntroduction(studioData.characters)
  ...
  await reportTaskProgress(job, 20, {
    ...
    stageLabel: 'Preparing dialogue analysis',
    displayMode: 'detail',
  })
      ...
      const voiceLine = typeof voiceLineModel.upsert === 'function'
        ...
        : (
          process.env.NODE_ENV === 'test'
            ...
            : (() => { throw new Error('studioVoiceLine.upsert unavailable') })()
        )
  ...
  await reportTaskProgress(job, 96, {
    ...
    stageLabel: 'Dialogue analysis saved',
    displayMode: 'detail',
  })
