...
export async function handleScreenplayConvertTask(job: Job<TaskJobData>) {
  ...
  await reportTaskProgress(job, 10, {
    ...
    stageLabel: 'Preparing screenplay',
    displayMode: 'detail',
  })
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    const stepTitle = `Screenplay ${stepIndex}/${total}`
  ...
  await reportTaskProgress(job, 96, {
    ...
    stageLabel: 'Screenplay saved',
    displayMode: 'detail',
    message: `${successCount}/${total} scenes done`,
  })
