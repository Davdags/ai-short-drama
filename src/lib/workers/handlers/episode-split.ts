...
import { createTextMarkerMatcher } from '@/lib/studio/story-to-script/clip-matching'
...
export async function handleEpisodeSplitTask(job: Job<TaskJobData>) {
  ...
  if (!content || content.length < 100) {
    throw new Error('The text is too short. At least 100 characters are needed')
  }
  ...
  if (project.mode !== 'studio') {
    throw new Error('Not a studio project')
  }

  const novelProject = await prisma.studioProject.findFirst({
    where: { projectId },
    select: { id: true },
  })
  if (!novelProject) {
    throw new Error('Studio data not found')
  }

  const userConfig = await getUserModelConfig(job.data.userId)
  const analysisModel = userConfig.analysisModel
  if (!analysisModel) {
    throw new Error('Please choose an analysis model in Model preferences first')
  }
  ...
  await reportTaskProgress(job, 20, {
    ...
    stageLabel: 'Preparing episode split',
    displayMode: 'detail',
  })
    ...
    for (let attempt = 1; attempt <= MAX_EPISODE_SPLIT_ATTEMPTS; attempt += 1) {
      try {
        ...
        await reportTaskProgress(job, 60, {
          ...
          stageLabel: attempt === 1 ? 'Reading episode split' : `Reading episode split (retry ${attempt - 1})`,
          displayMode: 'detail',
        })
        ...
        if (splitEpisodes.length === 0) {
          throw new Error('The episode split came back empty')
        }

        await reportTaskProgress(job, 80, {
          stage: 'episode_split_match',
          stageLabel: 'Matching episode text',
          displayMode: 'detail',
        })
        ...
        for (let idx = 0; idx < splitEpisodes.length; idx += 1) {
          ...
          if (episodeNumber === null) {
            throw new Error(`episode_${idx + 1} is missing a valid number`)
          }

          const title = typeof ep.title === 'string' ? ep.title.trim() : ''
          if (!title) {
            throw new Error(`episode_${idx + 1} is missing a title`)
          }
          ...
          if (!startMatch) {
            throw new Error(`episode_${idx + 1} startMarker could not be found`)
          }
          const endMatch = markerMatcher.matchMarker(endMarker, startMatch.endIndex)
          if (!endMatch) {
            throw new Error(`episode_${idx + 1} endMarker could not be found`)
          }
          ...
          if (rawStartIndex !== null && Math.abs(rawStartIndex - startMatch.startIndex) > 200) {
            throw new Error(`episode_${idx + 1} startIndex is too far from its marker`)
          }
          ...
          if (rawEndIndex !== null && Math.abs(rawEndIndex - endMatch.endIndex) > 200) {
            throw new Error(`episode_${idx + 1} endIndex is too far from its marker`)
          }
          ...
          if (startPos < searchFrom || endPos <= startPos || endPos > content.length) {
            throw new Error(`episode_${idx + 1} has an invalid range`)
          }

          const episodeContent = content.slice(startPos, endPos).trim()
          if (!episodeContent) {
            throw new Error(`episode_${idx + 1} matched no text`)
          }
  ...
  if (!episodes) {
    throw lastError || new Error('Could not match the episode boundaries')
  }
