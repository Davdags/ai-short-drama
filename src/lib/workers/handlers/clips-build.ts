...
import { createClipContentMatcher } from '@/lib/studio/story-to-script/clip-matching'
...
export async function handleClipsBuildTask(job: Job<TaskJobData>) {
  ...
  const clipModel = prisma.studioClip as unknown as {
    ...
    findMany: typeof prisma.studioClip.findMany
    deleteMany: typeof prisma.studioClip.deleteMany
  }
  ...
  if (episode.studioProjectId !== novelData.id) {
