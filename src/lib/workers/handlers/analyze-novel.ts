...
export async function handleAnalyzeNovelTask(job: Job<TaskJobData>) {
  ...
  const novelData = await prisma.studioProject.findUnique({
    where: { projectId },
    include: {
      characters: true,
      locations: true,
  ...
  const firstEpisode = await prisma.studioEpisode.findFirst({
    where: { studioProjectId: novelData.id },
  ...
  if (!contentToAnalyze.trim()) {
    throw new Error('Please add global settings or story text first')
  }
  ...
  const propPromptTemplate = buildPrompt({
    ...
    variables: {
      input: contentToAnalyze,
      props_lib_name: propsLibName || 'None',
    },
  })
  ...
  await reportTaskProgress(job, 80, {
    ...
    stageLabel: 'Prop analysis complete',
    displayMode: 'detail',
    stepId: 'analyze_props',
    stepTitle: 'Prop analysis',
    stepIndex: 3,
  ...
  await reportTaskProgress(job, 75, {
    ...
    stageLabel: 'Saving asset analysis',
    displayMode: 'detail',
  })
  ...
  for (const item of parsedCharacters) {
    ...
    const created = await prisma.studioCharacter.create({
      data: {
        studioProjectId: novelData.id,
        name,
  ...
  for (const item of parsedLocations) {
    ...
    const created = await prisma.studioLocation.create({
      data: {
        studioProjectId: novelData.id,
        name,
