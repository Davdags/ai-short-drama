...
import { resolveInsertPanelUserInput } from '@/lib/studio/insert-panel'
import {
  executePhase1,
  executePhase2,
  executePhase2Acting,
...
async function handleRegenerateStoryboardTextTask(job: Job<TaskJobData>) {
  ...
  const studioData = await prisma.studioProject.findUnique({
    where: { projectId },
    include: {
  ...
  if (!studioData) throw new Error('Studio data not found')
  if (!studioData.analysisModel) throw new Error('Analysis model not configured')
  const normalizedStudioData = {
    ...studioData,
    analysisModel: studioData.analysisModel,
    locations: studioData.locations.filter((item) => readAssetKind(item as unknown as Record<string, unknown>) !== 'prop'),
    props: studioData.locations
  ...
  const finalPanels = await withInternalLLMStreamCallbacks(
    ...
    async () =>
      await runStoryboardPhasesForClip({
        ...
        studioData: normalizedStudioData,
        projectId,
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.studioPanel.deleteMany({ where: { storyboardId } })
    await tx.studioStoryboard.update({
      where: { id: storyboardId },
...
async function handleInsertPanelTask(job: Job<TaskJobData>) {
  ...
  if (!projectData) throw new Error('Studio data not found')
  ...
  const propsDescription = projectProps
    ...
    .map((prop) => `${prop.name}: ${prop.summary || 'No description'}`)
    .join('\n') || 'None'
