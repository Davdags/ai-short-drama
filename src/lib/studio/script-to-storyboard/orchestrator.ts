...
export async function runScriptToStoryboardOrchestrator(
  ...
  const charactersLibName = (studioData.characters || []).map((c) => c.name).join(', ') || '无'
  const locationsLibName = (studioData.locations || []).map((l) => l.name).join(', ') || '无'
  ...
  const phase1Results = await mapWithConcurrency(
    ...
    async (clip, i) => {
      ...
      const filteredAppearanceList = getFilteredAppearanceList(studioData.characters || [], clipCharacters)
      const filteredFullDescription = getFilteredFullDescription(studioData.characters || [], clipCharacters)
      const filteredPropDescription = compileAssetPromptFragments(buildPromptAssetContext({
        characters: [],
        locations: [],
        props: studioData.props || [],
        clipCharacters: [],
        clipLocation: null,
        clipProps,
